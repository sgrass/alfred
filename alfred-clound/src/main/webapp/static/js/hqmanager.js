...
function fnFormatDetails(oTable, nTr) {
    ...
    sOut += '<tr><td>'+jQuery.i18n.prop("managers.email")+':<input type="email" name="email" class="form-control" value="' + email + '" '+disabled+'></td>';
    sOut += '<td>'+jQuery.i18n.prop("managers.password")+': <input type="password" name="password" id="passowrd" class="form-control " value="' + password + '"></td><td>'+jQuery.i18n.prop("managers.confirmPassword")+' : <input type="password" name="confirm_password" class="form-control " value="' + password + '"></td></tr>';
    sOut += '<tr><td>'+jQuery.i18n.prop("managers.firstName")+': <input type="text" class="form-control "value="' + firstName + '"></td>';
    sOut += '<td>'+jQuery.i18n.prop("managers.lastName")+': <input type="text" class="form-control "value="' + lastName + '"></td><td>'+jQuery.i18n.prop("managers.multipleRestaurant")+':'+restaurantName+'</td></tr>';
    sOut+='  <tr><td><td><a class="edit" href=""><button type="button" data="' + id + '"  class="btn btn-info"><i class="fa fa-save"></i>'+jQuery.i18n.prop("managers.saveBut")+'</button></a></td></tr>';
    sOut += '</table></form>';
    return sOut;
}
...
$(document).ready(function () {
    ...
    nCloneTh.innerHTML =jQuery.i18n.prop("managers.edit");
	        ...
	        // ajax 请求数据录入
	    		...
	    		$.ajax({
	                ...
	                success: function(data){
	               	 if(data.flag==false){
	               		bootbox.alert(jQuery.i18n.prop("managers.error.same"));
	                         return;
	               	 }else{
	               		$.ajax({
	            			type : "post",
	            			...
	            			error : function() {
	            				bootbox.alert(jQuery.i18n.prop("managers.error.add"));
	            			}
	            		});
	    		...
	    		$.ajax({
	     			...
	     			error : function() {
	     				bootbox.alert(jQuery.i18n.prop("managers.error.modify"));
	     			}
	     		});
                	...
                	if(id==undefined){
                		...
                		addFlag=true;
                	     ...
                	     $.ajax({
                 			...
                 			error : function() {
                 				bootbox.alert(jQuery.i18n.prop("managers.error.delete"));
                 			}
                 		});
                	}
                }              
