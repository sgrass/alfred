	...
	<section id="main-content">
		<section class="wrapper">
	      ...
	      <button id="editable-sample_new" class="btn btn-primary" onclick="$('#newPanel').show();">
	          <spring:message code="product.button.addnew"/> <i class="fa fa-plus"></i>
	      </button>
      </div>
			<div class="row">
				<div class="col-sm-6">
					...
					<section class="panel" id="newPanel" style="display:none">
						...
						<header class="panel-heading"><spring:message code="modifier.category.new"/>
							<span class="tools pull-right"> 
								<button type="button" class="btn btn-default btn-xs" onclick="$('#newPanel').hide();"><spring:message code="product.button.cancel"/></button>
						...
						<div class="panel-body">
							...
							<table class="table">
								<tr>
									<td align="right"><spring:message code="modifier.category.name"/>:</td>
									<td><input type="text" class="form-control small" id="categoryName" name="categoryName"/></td>
								</tr>
								<tr>
									<td align="right"><spring:message code="modifier.category.template"/>:</td>
									<td>
								...
								<tr>
									<td align="right"><spring:message code="product.status"/>:</td>
									<td>
								...
								<tr>
									<td align="center" colspan="2">
										<button type="button" class="btn btn-default btn-sm" onclick="saveNewModifier()"><spring:message code="product.button.save"/></button>
									</td>
								</tr>
							</table>
							</form>
					...
					<section class="panel">
						<header class="panel-heading">
							${mv.categoryName}
							<span class="tools pull-right"> 
								<button type="button" class="btn btn-default btn-xs" onclick="addNewTr(${i.index},${mv.id})"><spring:message code="product.button.addnew"/></button>
								<button type="button" class="btn btn-default btn-xs" onclick="editModifierCategory(${i.index},this)"><spring:message code="product.button.edit"/></button>
								<a href="javascript:;" class="fa fa-chevron-down"></a> 
							</span>
							<span class="tools pull-right" style="display:none"> 
								<button type="button" class="btn btn-default btn-xs" onclick="cancelModifierCategory(${i.index},this)"><spring:message code="product.button.cancel"/></button>
								<a href="javascript:;" class="fa fa-chevron-down"></a> 
							</span>
						</header>
						<div class="panel-body">
							...
							<table class="table" id="mTab${i.index}" style="display:none">
								...
								<tr>
									<td align="right">
										<button type="button" class="btn btn-default btn-sm" onclick="updateModifierCategory(${i.index},'update')"><spring:message code="product.button.update"/></button>
									</td>
									<td align="left">
										<button type="button" class="btn btn-default btn-sm" onclick="updateModifierCategory(${i.index},'delete')"><spring:message code="product.button.delete"/></button>
									</td>
								</tr>
							</table>
							</form>
							...
							<table class="table" id="tab${i.index}">
								<thead>
									<tr>
										...
										<th><spring:message code="product.item.price"/></th>
										<th><spring:message code="modifier.active"/></th>
										<th><spring:message code="modifier.default"/></th>
										<th><spring:message code="product.button.edit"/></th>
										<th><spring:message code="product.button.delete"/></th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td><a class="edit" href="javascript:;" onclick="editItemCategory(this)"><spring:message code="product.button.edit"/></a></td>
										<td><a class="delete" href="javascript:;" onclick="deleteModifierItemCategory(${m.id})"><spring:message code="product.button.delete"/></a></td>
									</tr>
									
									<tr style="display:none">
										<td>
											<input type="text" class="form-control small" name="modifierName" value="${m.modifierName}"/>
											<label name="icn" class="error" style="display:none"><spring:message code="modifier.err.add"/></label>
										</td>
										<td>
											<input type="text" class="form-control small" name="price" value="${m.price}"/>
											<label name="priceErr" class="error" style="display:none"><spring:message code="product.validate.decimal"/></label>
										</td>
										<td>
											<select class="form-control small" name="isActive">
										...
										<td><a class="edit" href="javascript:;" onclick="updateModifier('#m${i.index}${j.index}',this)"><spring:message code="product.button.save"/></a></td>
										<td><a class="cancel" href="javascript:;" onclick="cancelItemCategory(this)"><spring:message code="product.button.cancel"/></a></td>
									</tr>
									</form>
									</c:if>
									</c:forEach>
  ...
  <script src="${staticPath}/jquery.i18n.properties-min-1.0.9.js"></script>
	...
	<script>
	  ...
	  //左边菜单展开
		...
		function saveNewModifier() {
			var validator=$("#newModForm").validate({
			  rules : {
				...
				messages : {
					categoryName : jQuery.i18n.prop("modifier.category.err.add"),
				}
	    });
	    ...
	    bootbox.confirm(jQuery.i18n.prop("commom.delete.prompt"), function(result) {
	      if (result) {
    ...
    function addNewTr(tabId, mcId) {
			tr = $('<tr></tr>');
			td1 = $("<td></td>").append('<input type="text" class="form-control small" name="modifierName" /><label name="icn" class="error" style="display:none">'+jQuery.i18n.prop("modifier.err.add")+'</label>'+
					'<input type="hidden" name="categoryId" value="'+mcId+'">');
			td2 = $("<td></td>").append('<input type="text" class="form-control small" name="price" /><label name="priceErr" class="error" style="display:none">'+jQuery.i18n.prop("commom.verify.decimal")+'</label>');
			td3 = $("<td></td>").append('<select class="form-control small" name="isActive">'+
					'<option value="1">normal</option>'+
					'<option value="0">disable</option>'+
					'</select>');
			td4 = $("<td></td>").append('<input type="checkbox"  name="isActive" />');
			td5 = $("<td></td>").append('<a class="edit" href="javascript:;" onclick="insertModifier(this)">'+jQuery.i18n.prop("button.save")+'</a>');
			td6 = $("<td></td>").append('<a class="cancel" href="javascript:;" onclick="removeInsert(this)">'+jQuery.i18n.prop("button.cancel")+'</a></td>');
			tr.append(td1);
			tr.append(td2);
			tr.append(td3);
			tr.append(td4);
