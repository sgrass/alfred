				...
				<div class="col-sm-12">
					<section class="panel">
						<header class="panel-heading" style="height:67px">
							<div style="text-transform: capitalize;"><spring:message code="item.modifier.name"/>
              <div class="btn-group pull-right">
              	<button id="editable-sample_new" class="btn btn-primary" onclick="location.replace('${contextPath}/itemModifier/querySubCateModifier')"><i class="fa"></i><spring:message code="item.modifier.goback"/></button>
              </div>
             	</div>
						</header>
						<div class="panel-body">
							...
							<table class="table table-striped table-hover table-bordered" id="im-table">
								<thead>
									<tr>
										<th><spring:message code="product.item.name"/></th>
										<th><spring:message code="product.item.description"/></th>
										<th><spring:message code="item.modifier.name"/></th>
								...
								<tbody>
									<c:forEach var="itemDetail" items="${itemDetailList}" varStatus="i">
										<tr class="">
											...
											<td>
												<div id="save${i.index}" style="display:none"><a class="edit" href="javascript:;" onclick="saveItemModifier(${itemDetail.id},${itemDetail.itemCategoryId},${i.index})"><spring:message code="product.button.save"/></a></div>
											</td>
										</tr>
									</c:forEach>
								</tbody>
