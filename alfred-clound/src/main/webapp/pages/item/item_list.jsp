			...
			<!-- page start-->
             <div class="row">
             	...
             	<div class="panel-body">
                        <form id="fileupload" action="${contextPath}/importExcel/importItems" method="POST" enctype="multipart/form-data">
                            
                           <div class="form-group">
                                <label class="control-label col-md-3"><spring:message code="product.item.import"></spring:message> </label>
                                <div class="col-md-4">
                                    <input type="file" class="default"  name="excelFile" required accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" />
                                </div>
                            </div>
                                   <button type="submit" class="btn btn-info"><i class="fa fa-upload"></i> <spring:message code="product.item.button.upload"></spring:message></button>
                        </form>
                    </div>
             	</section>
             	</div>
				...
				<div class="col-sm-12">
                    ...
                    <header class="panel-heading" style="height:67px"> 
							...
							<div class="btn-group pull-right">
								<button id="editable-sample_new" class="btn btn-primary" onclick="addItemDetail(true)"><i class="fa fa-plus"></i> <spring:message code="product.item.name"></spring:message></button>
							</div>
							</div>
						</header>
						<div class="panel-body">
							<div class="row" style="margin-bottom: 40px;">
								<form class="form-inline" method="post" id="queryItemForm" action="${contextPath}/item/queryAll">
									...
									<div class="form-group" >
										<label class="col-lg-4 control-label" style="margin-top: 8px;white-space:nowrap;"><spring:message code="product.sub.category.name"></spring:message>:</label>
										<div class="col-lg-8">
								...
								<tbody>
									<tr id="addTr" style="display:none">
										<td colspan="6" class="details">
											<form id="addNewForm" action="${contextPath}/item/insert" method="post" enctype="multipart/form-data">
									...
									<c:forEach var="item" items="${itemList}" varStatus="i">
										...
										<tr id="tr${i.index}" style="display: none">
											<td colspan="6" class="details">
												<form id="form${i.index}" action="${contextPath}/item/updateById" method="post" enctype="multipart/form-data">
...
// 			$(obj).parent().parent().next().show();
		...
		function deleteItem(itemId) {
			bootbox.confirm("Are you sure?", function(result) {
				if (result) {
					location.replace("${contextPath}/item/deleteById?id="+itemId);	
		    }               
	    });
		}
