				...
				<div class="col-sm-12">
					<section class="panel">
						<header class="panel-heading">
							<div class="row">
								<div style="text-transform: capitalize;">BOH REPORT
						...
						<div class="panel-body">
							...
							<div class="adv-table">
								<table class="display table table-bordered table-striped" id="bohhold-table">
									<thead>
										<tr>
											<th>Bill Number</th>
											<th>Customer</th>
...
$(document).ready(function(){
    ...
    $(".dropdown-menu li").click(function(){
    	if($(this).text()=='Export PDF'){
        	...
        	$(".form-inline").attr("action","/report/exportBohSettPdf");
        	$(".form-inline").submit();
    		...
    		$("#startTime").val($('input[name="daterangepicker_start"]').val());
    		$("#endTime").val( $('input[name="daterangepicker_end"]').val());
    		$(".form-inline").attr("action","/report/exportBohSett");
        	$(".form-inline").submit();
    ...
    $("#bohli").addClass("active");
    
    var oTable = $('#bohhold-table').dataTable({
        "bServerSide" : true,
		"sAjaxSource" : "/report/ajaxBohSettlementJson",//mvc后台ajax调用接口。
		'bPaginate' : true,//是否分页。
		...
		"fnCreatedRow": function( nRow, aData, iDataIndex ) {
			//var dateFmt=new Date(parseInt(aData.businessDate)).format("MM-dd-yyyy");
			 $('td:eq(3)', nRow).html(aData.status==0?'NoPaid':'Prepaid');
             $('td:eq(8)', nRow).html(moment(aData.paidDate).format("MM-DD-YYYY"));
			   },
 ...
 function reflashTableList(){
     ...
     oSettings.sAjaxSource="/report/ajaxBohSettlementJson";
