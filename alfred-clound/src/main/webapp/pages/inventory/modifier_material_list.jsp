...
<body>
									...
									<c:forEach var="modifier" items="${modifierList}" varStatus="i">
										...
										<tr id="tr${i.index}" style="display: none">
											<td colspan="3" class="details" align="left">
												<form id="form${i.index}" action="${contextPath}/modifierMaterial/insert" method="post" onsubmit="return isSubmit('form${i.index}')">
													<table border="0" width="60%">
														<c:forEach var="material" items="${materialList}" varStatus="j">
															...
															<tr>
																...
																<td width="75%">
																	<input type="text" class="form-control small" name="useQty${j.index}" value="${useQty }"/>
																	<label name="qtyErr${j.index}" class="error" style="display:none">decimal point is limited to 2 digits.</label>
																</td>
	...
	<script>
		...
		function isSubmit(formId) {
			var reg =/^[0-9]+([.]\d{1,2})?$/;
			flag = true;
			$($("#"+formId).find("input[name=checkedIds]")).each(function (){
				if ($(this).prop("checked")) {
					var checkId = $(this).val();
					var qtyValue = $("#"+formId).find("input[name=useQty"+checkId+"]").val();
					if (!reg.test(qtyValue)) {
						$("#"+formId).find("label[name=qtyErr"+checkId+"]").show();
						flag = false;
					}
				}
			});
